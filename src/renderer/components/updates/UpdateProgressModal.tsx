            ...
            <div className="space-y-6 text-center py-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                    <div className="liquid-glass-modal-inset p-3 rounded-2xl">
                        ...
                        <span className="liquid-glass-modal-text block text-xl font-bold">{percent.toFixed(0)}%</span>
                    </div>
                    <div className="liquid-glass-modal-inset p-3 rounded-2xl">
                        <span className="liquid-glass-modal-text-muted block">{t('updates.downloading.speed')}</span>
                        <span className="liquid-glass-modal-text block text-xl font-bold">{speed.toFixed(1)} MB/s</span>
                    </div>
                    <div className="liquid-glass-modal-inset p-3 rounded-2xl">
                        <span className="liquid-glass-modal-text-muted block">{t('updates.downloading.downloaded')}</span>
                        <span className="liquid-glass-modal-text block text-xl font-bold">{transferred.toFixed(1)} / {total.toFixed(1)} MB</span>
                    </div>
                </div>
