  ...
  return (
    <div className={`space-y-2 ${className}`.trim()} data-testid="health-support-entrypoint">
      <button
        ...
        className={`inline-flex min-h-[44px] items-center gap-2 rounded-2xl border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs font-semibold text-amber-700 transition-transform active:scale-[0.98] active:bg-amber-500/15 dark:text-amber-300 ${buttonClassName}`.trim()}
      >
