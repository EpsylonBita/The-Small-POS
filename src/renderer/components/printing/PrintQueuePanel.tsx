  ...
  return (
    <div className="rounded-2xl border liquid-glass-modal-border bg-white/5 dark:bg-gray-800/10 px-4 py-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <Printer className="h-4 w-4 text-yellow-600 dark:text-yellow-300" />
          ...
          {pausedPrinterProfileIds.length > 0 && (
            <p className="mt-2 text-[11px] text-yellow-800 dark:text-yellow-200">
              {t('settings.printQueue.pausedProfiles', 'Profile pauses active for configured printers')}
            </p>
          )}
          {failedCount > 0 && (
            <p className="mt-2 text-[11px] text-red-700 dark:text-red-200">
        ...
        <div className="grid w-full shrink-0 grid-cols-1 gap-2 md:w-48">
          ...
          <button
            ...
            className="inline-flex w-full min-h-[44px] items-center justify-center gap-2 rounded-2xl border border-red-300 bg-red-50 px-3 py-2 text-xs font-semibold text-red-700 transition active:bg-red-100 disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-100 disabled:text-slate-500 dark:border-red-400/30 dark:bg-red-500/10 dark:text-red-100 dark:active:bg-red-500/20 dark:disabled:border-white/10 dark:disabled:bg-white/5 dark:disabled:text-slate-400"
          >
      ...
      <div className="mt-4 overflow-hidden rounded-2xl border border-slate-200 dark:border-white/10">
        <div className="grid grid-cols-[minmax(0,1.2fr)_minmax(0,0.9fr)_minmax(0,1fr)_auto] gap-3 bg-slate-100 px-3 py-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-600 dark:bg-white/5 dark:text-slate-400">
        ...
        <div className="max-h-72 overflow-y-auto scrollbar-hide divide-y divide-slate-200 dark:divide-white/5">
          {jobs.length === 0 ? (
            ...
            visibleJobs.map((job) => {
              ...
              return (
                <div
                  key={job.id}
                  className="grid grid-cols-[minmax(0,1.2fr)_minmax(0,0.9fr)_minmax(0,1fr)_auto] gap-3 px-3 py-3 text-sm text-slate-700 dark:text-slate-200"
                >
                  <div className="min-w-0">
                    <div className="truncate font-medium text-slate-900 dark:text-white">
                      {getJobTypeLabel(job.entityType)}
                    </div>
                    <div className="truncate text-xs text-slate-600 dark:text-slate-400">{getJobReferenceLabel(job)}</div>
                    <div className="truncate text-[11px] text-slate-500 dark:text-slate-500">
                      {formatTimestamp(job.createdAt)}
                    </div>
                    {issueLabel && (
                      <div className={`mt-1 truncate text-[11px] ${job.warningMessage ? 'text-yellow-800 dark:text-yellow-200' : 'text-red-700 dark:text-red-200'}`}>
                        {issueLabel}
                      </div>
                    )}
                  </div>
                  <div className="flex items-center text-xs uppercase tracking-[0.14em] text-slate-600 dark:text-slate-300">
                  ...
                  <div className="flex items-center gap-2">
                    {isActive && (
                      <button
                        ...
                        className="inline-flex min-h-[44px] items-center justify-center gap-1 rounded-xl border border-red-300 bg-red-50 px-2 py-1 text-xs font-semibold text-red-700 transition active:bg-red-100 disabled:cursor-not-allowed disabled:opacity-60 dark:border-red-400/30 dark:bg-red-500/10 dark:text-red-100 dark:active:bg-red-500/20"
                      >
                    ...
                    {canRetry && (
                      <button
                        ...
                        disabled={busyJobId === job.id}
                        className="inline-flex min-h-[44px] items-center justify-center gap-1 rounded-xl border border-emerald-300 bg-emerald-50 px-2 py-1 text-xs font-semibold text-emerald-700 transition active:bg-emerald-100 disabled:cursor-not-allowed disabled:opacity-60 dark:border-emerald-400/30 dark:bg-emerald-500/10 dark:text-emerald-100 dark:active:bg-emerald-500/20"
                      >
