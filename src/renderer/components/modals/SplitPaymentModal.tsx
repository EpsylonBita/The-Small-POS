...
interface SplitPaymentModalProps {
  ...
  initialMode?: TabMode; isGhostOrder?: boolean;
}
...
export const SplitPaymentModal: React.FC<SplitPaymentModalProps> = ({ isOpen, onClose, orderId, orderTotal, items, onSplitComplete, existingPayments = EMPTY_EXISTING_PAYMENTS, initialMode = 'by-amount', isGhostOrder = false }) => {
  ...
  const initializePortions = useCallback((mode: TabMode, amountDue: number) => { const half = round2(amountDue / 2); setPortions(mode === 'by-items' ? [createPortion(personLabel(0), 0), createPortion(personLabel(1), 0)] : [createPortion(personLabel(0), half), createPortion(personLabel(1), round2(amountDue - half))]); setActiveTab(mode); setReceiptMode('combined'); setItemAssignments({}); setOpenAssignmentItemIndex(null); setDiscountEditorPortionId(null); setDiscountDraftValue(''); }, [personLabel]);
  ...
  const addPerson = useCallback(() => { if (!processingPortionId && !isProcessing) setPortions((current) => [...current, createPortion(personLabel(current.length), 0)]); }, [isProcessing, personLabel, processingPortionId]);
  ...
  const recordPortionPayment = useCallback(async (portion: SplitPortion, paymentOrigin: PaymentOrigin, transactionRef?: string, terminalDeviceId?: string) => {
    ...
    return paymentId;
  }, [activeTab, appendCompletedPayment, bridge, orderId]);
  ...
  const MethodToggle: React.FC<{ portion: SplitPortion }> = ({ portion }) => {
    ...
    return <div className="flex gap-1 rounded-lg bg-white/5 p-0.5"><button type="button" disabled={locked} onClick={() => setPortionMethod(portion.id, 'cash')} className={`flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium transition-all ${portion.method === 'cash' ? 'border border-green-400/30 bg-green-500/20 text-green-400' : 'text-white/40 hover:text-white/60'} ${locked ? 'cursor-not-allowed opacity-70' : ''}`}><Banknote className="h-3.5 w-3.5" />{t('splitPayment.cash', 'Cash')}</button><button type="button" disabled={locked} onClick={() => void handleTerminalCardPayment(portion.id)} className={`flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium transition-all ${portion.method === 'card' ? 'border border-blue-400/30 bg-blue-500/20 text-blue-400' : 'text-white/40 hover:text-white/60'} ${locked ? 'cursor-not-allowed opacity-70' : ''}`}>{portion.status === 'processing' ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <CreditCard className="h-3.5 w-3.5" />}{t('splitPayment.card', 'Card')}</button></div>;
  };
  const renderPortionDetails = (portion: SplitPortion) => <div className="space-y-2"><div className="flex items-center justify-between text-xs liquid-glass-modal-text-muted"><span>{t('modals.orderDetails.subtotal', { defaultValue: 'Subtotal' })}</span><span>&euro;{portion.grossAmount.toFixed(2)}</span></div><div className="flex items-center justify-between text-xs text-green-300"><span>{t('modals.orderDetails.discount', { defaultValue: 'Discount' })}</span><span>-&euro;{portion.discountAmount.toFixed(2)}</span></div><div className="flex items-center justify-between text-sm font-semibold text-emerald-400"><span>{t('splitPayment.payable', { defaultValue: 'Payable' })}</span><span>&euro;{portion.amount.toFixed(2)}</span></div>{portion.status === 'paid' ? <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-xs text-emerald-200">{t('modals.orderDetails.paid', { defaultValue: 'Paid' })} • {t(portion.method === 'card' ? 'modals.orderDetails.card' : 'modals.orderDetails.cash', { defaultValue: portion.method === 'card' ? 'Card' : 'Cash' })}{portion.paymentOrigin === 'terminal' ? ` • ${t('splitPayment.terminalApproved', { defaultValue: 'Terminal' })}` : ''}</div> : portion.status === 'processing' ? <div className="rounded-lg border border-blue-500/30 bg-blue-500/10 px-3 py-2 text-xs text-blue-200">{t('splitPayment.waitingForApproval', { defaultValue: 'Waiting for card approval on the payment terminal...' })}</div> : <><button type="button" disabled={portion.grossAmount <= 0.009} onClick={() => openDiscountEditor(portion.id)} className={`flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium transition-all ${portion.grossAmount > 0.009 ? 'border border-yellow-500/30 bg-yellow-500/10 text-yellow-300 hover:bg-yellow-500/15' : 'cursor-not-allowed bg-gray-500/20 text-gray-500'}`}><BadgePercent className="h-3.5 w-3.5" />{t('splitPayment.discount', { defaultValue: 'Discount' })}</button>{discountEditorPortionId === portion.id && <div className="flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 p-2"><div className="relative flex-1"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-white/40">&euro;</span><input type="number" step="0.01" min="0" max={portion.grossAmount} value={discountDraftValue} onChange={(event) => setDiscountDraftValue(event.target.value)} className="w-full rounded-lg border border-white/20 bg-white/10 py-2 pl-7 pr-3 text-sm liquid-glass-modal-text focus:border-emerald-400/50 focus:outline-none" placeholder="0.00" /></div><button type="button" onClick={() => saveDiscount(portion.id)} className="rounded-lg border border-emerald-500/30 bg-emerald-600/20 px-3 py-2 text-xs font-semibold text-emerald-300">{t('common.actions.apply', { defaultValue: 'Apply' })}</button><button type="button" onClick={() => { setDiscountEditorPortionId(null); setDiscountDraftValue(''); }} className="rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-xs font-semibold text-white/70">{t('common.actions.cancel', { defaultValue: 'Cancel' })}</button></div>}</>}</div>;
