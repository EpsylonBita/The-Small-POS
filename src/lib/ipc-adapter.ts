...
function normalizeSettingsGetRequest(
  ...
  if (extraDefault !== undefined && out.defaultValue === undefined)
    out.defaultValue = extraDefault;
  return out;
}
...
function buildSettingsGetInvoke(
  ...
  const normalized = normalizeSettingsGetRequest(
    request ?? {},
    key,
    defaultValue,
  );
...
function buildDiagnosticsExportArgs(
...
function normalizeFinancialQueueItem(
  raw: unknown,
): SyncFinancialQueueItem | null {
  ...
  const retryCount = toSyncFinancialRetryCount(
    item.retryCount ?? item.attempts,
  );
  ...
  const parentShiftSyncStatus = toSyncFinancialString(
    item.parentShiftSyncStatus,
    "",
  );
  const parentShiftQueueStatus = toSyncFinancialString(
    item.parentShiftQueueStatus,
    "",
  );
  const dependencyBlockReason = toSyncFinancialString(
  ...
  const parentShiftQueueId =
    typeof parentShiftQueueIdRaw === "number"
      ...
      : typeof parentShiftQueueIdRaw === "string" &&
          parentShiftQueueIdRaw.trim()
...
export interface ShiftPrintCheckoutParams {
  shiftId: string;
  roleType?: string;
  terminalName?: string;
...
export interface PlatformBridge {
  ...
  orders: {
    ...
    updateCustomerInfo(
      payload: OrderCustomerInfoUpdateParams,
    ): Promise<IpcResult>;
    convertPickupToDelivery(
      payload: PickupToDeliveryConversionParams,
    ): Promise<IpcResult>;
  ...
  receipt: {
    samplePreview(
      payload?: ReceiptSamplePreviewRequest,
    ): Promise<ReceiptSamplePreviewResponse>;
  };
  ...
  sync: {
    ...
    removeInvalidOrders(
      orderIds: string[],
    ): Promise<SyncRemoveInvalidOrdersResponse>;
  ...
  settings: {
    ...
    updateLocal(
      request: SettingsUpdateLocalInput,
      value?: unknown,
    ): Promise<IpcResult>;
  ...
  terminalConfig: {
    ...
    getSetting(
      request: TerminalConfigGetSettingInput,
      key?: string,
    ): Promise<unknown>;
  ...
  appointments: {
    list(
      params?: AppointmentBridgeListParams,
  ...
  reservations: {
    list(
      params?: ReservationBridgeListParams,
    ): Promise<AdminApiBridgeResponse<any>>;
  ...
  services: {
    list(
      params?: ServiceBridgeListParams,
    ): Promise<AdminApiBridgeResponse<any>>;
    categories(
      params?: ServiceCategoryBridgeListParams,
    ): Promise<AdminApiBridgeResponse<any>>;
  };

  resources: {
    list(
      params?: ResourceBridgeListParams,
    ): Promise<AdminApiBridgeResponse<any>>;
  };
  ...
  staffSchedule: {
    ...
    clock(
      payload: StaffScheduleClockPayload,
    ): Promise<AdminApiBridgeResponse<any>>;
  };
  ...
  shifts: {
    ...
    getCheckInEligibility(
      branchId: string,
      terminalId: string,
    ): Promise<ShiftCheckInEligibility>;
  ...
  reports: {
    ...
    getEndOfDayStatus(params: {
      branchId: string;
    }): Promise<EndOfDayStatusResponse>;
    getDailyStaffPerformance(params: {
      branchId: string;
      date?: string;
    }): Promise<any>;
    printZReport(params: {
  ...
  hardware: {
    ...
    displayShowItem(params: {
      name: string;
      price: number;
      qty?: number;
      currency?: string;
    }): Promise<IpcResult>;
    displayShowTotal(params: {
      subtotal: number;
      tax?: number;
      total: number;
      currency?: string;
  ...
  branchData: {
    getTables(
      params?: TableBridgeListParams,
    ): Promise<AdminApiBridgeResponse<any>>;
    getStaffSchedule(
      params: StaffScheduleListParams,
    ): Promise<AdminApiBridgeResponse<any>>;
...
export const CHANNEL_MAP: Record<string, string> = {
  ...
  "shift:get-active-cashier-by-terminal-loose":
  ...
  "shift:get-staff-payment-total-for-date":
    "shifts.getStaffPaymentTotalForDate",
...
export class TauriBridge implements PlatformBridge {
  ...
  private static readonly COMMAND_OVERRIDES: Record<string, string> = {
    ...
    "printer:list-jobs": "print_list_jobs",
  ...
  auth = {
    ...
    setupPin: (request: AuthSetupPinInput) =>
      this.inv("auth:setup-pin", buildAuthSetupPinArg(request)),
  ...
  staffAuth = {
    ...
    hasAnyPermission: (ps: string[]) =>
      this.inv("staff-auth:has-any-permission", ps),
  ...
  orders = {
    ...
    getByCustomerPhone: (phone: string) =>
      this.inv("order:get-by-customer-phone", phone),
    ...
    updateFinancials: (payload: OrderFinancialsUpdateParams) =>
      this.inv("order:update-financials", payload),
    ...
    updatePreparation: (id: string, s: string, p: number, m?: string) =>
      this.inv("order:update-preparation", id, s, p, m),
    updateType: (id: string, t: string) => this.inv("order:update-type", id, t),
    fetchItemsFromSupabase: (id: string) =>
      this.inv("order:fetch-items-from-supabase", id),
    getConflicts: () => this.inv("orders:get-conflicts"),
    resolveConflict: (cid: string, s: string, d?: any) =>
      this.inv("orders:resolve-conflict", cid, s, d),
  ...
  payments = {
    ...
    getPaidItems: (orderId: string) =>
      this.inv("payment:get-paid-items", orderId),
    printSplitReceipt: (paymentId: string) =>
      this.inv("payment:print-split-receipt", paymentId),
  };
  ...
  sync = {
    ...
    getUnsyncedFinancialSummary: () =>
      this.inv("sync:get-unsynced-financial-summary"),
    validateFinancialIntegrity: () =>
      this.inv("sync:validate-financial-integrity"),
    ...
    removeInvalidOrders: (orderIds: string[]) =>
      this.inv("sync:remove-invalid-orders", orderIds),
    fetchSuppliers: () => this.inv("sync:fetch-suppliers"),
    fetchAnalytics: () => this.inv("sync:fetch-analytics"),
    fetchOrders: (opts?: any) =>
      opts === undefined
        ? this.inv("sync:fetch-orders")
        : this.inv("sync:fetch-orders", opts),
    fetchRooms: (opts?: any) => this.inv("sync:fetch-rooms", opts),
    updateRoomStatus: (roomId: string, status: string) =>
      this.inv("sync:update-room-status", roomId, status),
    fetchDriveThru: (opts?: any) => this.inv("sync:fetch-drive-thru", opts),
    updateDriveThruOrderStatus: (id: string, status: string) =>
      this.inv("sync:update-drive-thru-order-status", id, status),
  };
  ...
  customers = {
    ...
    update: (id: string, u: Partial<Customer>, v: number) =>
      this.inv("customer:update", id, u, v),
    updateBanStatus: (id: string, b: boolean) =>
      this.inv("customer:update-ban-status", id, b),
    addAddress: (id: string, a: Partial<CustomerAddress>) =>
  ...
  settings = {
    ...
    getLocal: (k?: string) =>
      k ? this.inv("settings:get-local", k) : this.inv("settings:get-local"),
    updateLocal: (request: SettingsUpdateLocalInput, value?: unknown) =>
      this.inv(
        "settings:update-local",
        ...buildSettingsUpdateLocalArgs(request, value),
      ),
  ...
  terminalConfig = {
    ...
    getSetting: (request: TerminalConfigGetSettingInput, key?: string) =>
      this.inv(
        ...
        ...buildTerminalConfigGetSettingArgs(request, key),
      ),
  ...
  appointments = {
    ...
    get: (appointmentId: string) =>
      this.adminFetch(`/api/pos/appointments/${appointmentId}`),
    create: (payload: Record<string, unknown>) =>
      this.adminFetch("/api/pos/appointments", {
        method: "POST",
        body: payload,
      }),
  ...
  reservations = {
    ...
    create: (payload: Record<string, unknown>) =>
      this.adminFetch("/api/pos/reservations", {
        method: "POST",
        body: payload,
      }),
  ...
  staffSchedule = {
    ...
    checkAvailability: (params: StaffScheduleCheckAvailabilityParams) =>
      this.adminFetch(
        `/api/pos/staff-schedule/check${buildQueryString(params)}`,
      ),
    clock: (payload: StaffScheduleClockPayload) =>
      this.adminFetch("/api/pos/staff-schedule/clock", {
        method: "POST",
        body: payload,
      }),
  };
  ...
  shifts = {
    ...
    getActiveByTerminalLoose: (t: string) =>
      this.inv("shift:get-active-by-terminal-loose", t),
    getActiveCashierByTerminal: (b: string, t: string) =>
      this.inv("shift:get-active-cashier-by-terminal", b, t),
    getCheckInEligibility: (b: string, t: string) =>
      this.inv(
        "shift:get-check-in-eligibility",
        b,
        t,
      ) as Promise<ShiftCheckInEligibility>,
    getActiveCashierByTerminalLoose: (t: string) =>
      this.inv("shift:get-active-cashier-by-terminal-loose", t),
    getSummary: (id: string, opts?: { skipBackfill?: boolean }) =>
    ...
    getStaffPaymentTotalForDate: (staffId: string, date: string) =>
      this.inv("shift:get-staff-payment-total-for-date", staffId, date),
    getScheduledShifts: (params: any) =>
      this.inv("shift:get-scheduled-shifts", params),
    getTodayScheduledShifts: (branchId: string) =>
      this.inv("shift:get-today-scheduled-shifts", branchId),
    backfillDriverEarnings: (p: { shiftId?: string; date?: string }) =>
      this.inv("shift:backfill-driver-earnings", p),
  };
  ...
  deliveryZones = {
    ...
    validateLocal: (payload: any) =>
      this.inv("delivery-zone:validate-local", payload),
  };
  ...
  reports = {
    getTodayStatistics: (p: { branchId: string }) =>
      this.inv("report:get-today-statistics", p),
    getSalesTrend: (p: { branchId: string; days: number }) =>
      this.inv("report:get-sales-trend", p),
    getTopItems: (p: { branchId: string; date?: string; limit?: number }) =>
      this.inv("report:get-top-items", p),
    getWeeklyTopItems: (p: { branchId: string; limit?: number }) =>
      this.inv("report:get-weekly-top-items", p),
    getHourlySales: (p: { branchId: string; date?: string }) =>
      this.inv("report:get-hourly-sales", p),
    getPaymentMethodBreakdown: (p: { branchId: string; date?: string }) =>
      this.inv("report:get-payment-method-breakdown", p),
    getOrderTypeBreakdown: (p: { branchId: string; date?: string }) =>
      this.inv("report:get-order-type-breakdown", p),
    generateZReport: (p: { branchId: string; date?: string }) =>
      this.inv("report:generate-z-report", p),
    getEndOfDayStatus: (p: { branchId: string }) =>
      this.inv("report:get-end-of-day-status", p),
    getDailyStaffPerformance: (p: { branchId: string; date?: string }) =>
      this.inv("report:get-daily-staff-performance", p),
    printZReport: (p: {
      zReportId?: string;
      snapshot?: any;
      terminalName?: string;
  ...
  menu = {
    ...
    getSubcategoryIngredients: (id: string) =>
      this.inv("menu:get-subcategory-ingredients", id),
    getCombos: () => this.inv("menu:get-combos"),
    updateCategory: (id: string, u: any) =>
      this.inv("menu:update-category", id, u),
    updateSubcategory: (id: string, u: any) =>
      this.inv("menu:update-subcategory", id, u),
    updateIngredient: (id: string, u: any) =>
      this.inv("menu:update-ingredient", id, u),
  ...
  printer = {
    ...
    update: (id: string, u: Partial<PrinterConfig>) =>
      this.inv("printer:update", id, u),
    ...
    testDraft: (profileDraftOrPayload: any, sampleKind?: string) => {
      const payload =
        ...
        !(
          ...
          ? {
              profileDraft: profileDraftOrPayload,
    ...
    openCashDrawer: (id?: string, drawer?: 1 | 2) =>
      this.inv("printer:open-cash-drawer", id, drawer),
  ...
  ecr = {
    ...
    processPayment: (amt: number, o?: EcrPaymentOptions) =>
      this.inv("ecr:process-payment", amt, o),
    processRefund: (amt: number, o?: EcrRefundOptions) =>
      this.inv("ecr:process-refund", amt, o),
    voidTransaction: (tid: string, did?: string) =>
      this.inv("ecr:void-transaction", tid, did),
  ...
  loyalty = {
    ...
    getCustomerBalance: (customerId: string) =>
      this.inv("loyalty:get-balance", { customerId }),
    lookupByPhone: (phone: string) =>
      this.inv("loyalty:lookup-phone", { phone }),
    lookupByCard: (uid: string) => this.inv("loyalty:lookup-card", { uid }),
    earnPoints: (opts: {
      customerId: string;
      orderId?: string;
      amount: number;
    }) => this.inv("loyalty:earn", opts),
    redeemPoints: (opts: {
      customerId: string;
      points: number;
      orderId?: string;
    }) => this.inv("loyalty:redeem", opts),
    getTransactions: (customerId: string) =>
      this.inv("loyalty:transactions", { customerId }),
  };
  ...
  labels = {
    ...
    printBatch: (items: any[], lt?: string, pid?: string) =>
      this.inv("label:print-batch", items, lt, pid),
  };
  ...
  refunds = {
    ...
    getPaymentBalance: (paymentId: string) =>
      this.inv("refund:get-payment-balance", paymentId),
  };
  ...
  recovery = {
    ...
    restorePoint: (pointId: string) =>
      this.inv("recovery:restore-point", { pointId }),
    openDir: (path?: string) =>
      path
  ...
  const handler: ProxyHandler<any> = {
    get(_target, prop) {
      if (prop === "invoke") {
        return (...args: unknown[]) => {
          console.warn(
            `[BrowserStub] IPC invoke not available in browser:`,
            args,
          );
          return Promise.reject(
            new Error("IPC not available in browser environment"),
          );
        };
      }
      return new Proxy(
        {},
        {
          get(_t, method) {
            return (...args: unknown[]) => {
              console.warn(
                `[BrowserStub] ${String(prop)}.${String(method)} not available in browser`,
                args,
              );
              return Promise.reject(
                new Error(
                  `${String(prop)}.${String(method)} is not available in browser environment`,
                ),
              );
            };
          },
