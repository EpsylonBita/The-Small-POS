...
{
  "version": "1.2.1",
  "notes": "Release v1.2.1",
  "pub_date": "2026-02-19T12:00:00.000Z",
  "platforms": {
    "windows-x86_64": {
      "signature": "<minisign signature>",
      "url": "https://github.com/EpsylonBita/The-Small-POS/releases/download/v1.2.1/The Small POS_1.2.1_x64-setup.exe"
    }
  }
}
```
...
node scripts/generate-updater-manifest.mjs \
  ...
  --assetFileName "The Small POS_1.2.1_x64-setup.exe" \
  --signatureFilePath "src-tauri/target/release/bundle/nsis/The Small POS_1.2.1_x64-setup.exe.sig" \
  --outputPath "src-tauri/target/release/bundle/nsis/latest.json"
```

## Hotfix Note (v1.2.1)

- `v1.2.1` fixes a startup crash in `v1.2.0` (`TypeError: (e || '').trim is not a function`) caused by non-string runtime config values.
- If a terminal is stuck on `v1.2.0` and cannot open the app UI, manually reinstall `v1.2.1` once from the release page, then normal updater flow resumes.
