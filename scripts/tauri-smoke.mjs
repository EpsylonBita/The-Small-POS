...
const GATES = [
  ...
  {
    id: 'G7',
    name: 'Factory Reset Round-Trip',
    steps: [
      'Provision terminal credentials and verify settings_get_credential_status reports all true',
      'Create one local order and one pending sync item',
      'Export or record the pre-reset terminal/org/branch identifiers for evidence',
      'Run factory reset from the app',
      'Verify keyring credentials are removed',
      'Verify local_settings no longer contains admin URL, API key, or terminal identifiers',
      'Verify sync_queue and parity_sync_queue contain no pre-reset rows',
      'Verify renderer localStorage/session state is cleared',
      'Re-provision from onboarding',
      'Verify settings_get_credential_status reports all true again',
      'Verify no pre-reset operational state is visible after re-provisioning',
    ],
    critical: true,
  },
  {
  ...
  },
  {
    ...
    steps: [
      ...
      'Verify sync_queue or parity_sync_queue has z_report/z_reports entry with status: pending',
      ...
      'Verify parity_sync_queue has no pending z_reports blocker',
